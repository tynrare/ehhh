...
import { ShelfThingy } from './shelf_thingy.js';
 ...
 */
class EventThingy extends NanoThingy {
	/**
	 ...
	 */
	off(key) {
		const evt = this.cacheEvents.get(key);
		if (evt) {
	...
	pipe(type) {
		const id = this.on(type, (...args) => {
			const t = this.cacheEvents.get(id).type;
			this.emit(t, ...args);
		});
	}
	...
	registerListener(type, callback, func = 'on') {
		...
		const id = this.cacheEvents.add({ type: evt, callback, listener });
	...
	get lowlevel() {
		if (!this.cache.lowlevel) {
			this.cache.lowlevel = new EventEmitter();
		}
	 ...
	 */
	get cacheEvents() {
		return this.touch('events', () => new ShelfThingy());
	}
	...
	dispose() {
		...
		super.dispose();
		for (const k in this.cacheEvents.properties) {
			this.off(k);
		}
	}
}
